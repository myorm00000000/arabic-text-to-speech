...
class FeatureWriter(Writer):
    def write(self, wav_path, text_path, grid_path, file):
        ...
        with open(text_path, 'r') as ff:
            phonemes = FeatureExtractor().extract_from_sentences(ff.read(), arabic=False)
        temp_path = "temp.wav"
        for interval, phoneme in zip(intervals, phonemes):
            min_time = interval.minTime
            max_time = interval.maxTime
            FeatureWriter.cut(wav_path, min_time, max_time, temp_path)
            x, fs = sf.read(temp_path)
            f0, sp, ap = pw.wav2world(x, fs, frame_period=FeatureReader.frame_length)
            num_of_frames = int(math.ceil((max_time - min_time) / float(FeatureReader.frame_length)))
