...
from phoneme.findstress import *
...
def phonetise(text, arabic=True):
            ...
            if (not word in [u'-', u'sil']):
                ...
                for i in range(0, possibilities):
                    ...
                    for index, letter in enumerate(phones):
                        if (isinstance(letter, list)):
                            curIndex = round(i / iterations) % len(letter)
...
if (__name__ == "__main__"):
    ...
    (utterancesPronuncationsWithBoundaries, utterancesPronuncations, dict, _) = phonetise(inputFile.read())
    inputFile.close()
